function models = train_classes_hos_paris_20x1_smooth_greedycover(...
               classid, sharpness, alpha, K1, K2, nms_threshold, varargin)
                            
% HOS: Sort clusters by Paris score, greedily take
...
ip.parse(trainset, year, classid, sharpness, ...
                     alpha, K1, K2, nms_threshold, varargin{:});
opts = ip.Results;
...
d = load_cached_features_hos(dataset.trainset, dataset.year, ...
                         dataset.neg_image_ids{ind}, opts);
...
for boxid = 1:length(trainset_matrix)
  ...
  img_struct = load_cached_features_hos(dataset.trainset, ...
    dataset.year, sample_pos_image_ids{this_image_idx}, opts);
