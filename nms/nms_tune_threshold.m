function [best_thresh, best_ap, res] = nms_tune_threshold(model, testset, year)
...
for i = 1:length(threshs)
  thresh = threshs(i);
  ap_auc = compute_at_nms_thresh(model, boxes, thresh, VOCopts);
  res(i,:) = [thresh ap_auc];
  if ap_auc > best_ap
    best_thresh = thresh;
    best_ap = ap_auc;
    fprintf('!!! %s AP = %.3f @ thresh = %.3f\n', model.class, best_ap, thresh);
  end
end



function ap_auc = compute_at_nms_thresh(model, boxes, thresh, VOCopts)
